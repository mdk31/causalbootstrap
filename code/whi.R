...
whi_cblb <- whi_cblb[gamma == 0.8 & subsets == 10]
whi_cblb[, `:=`(method = car::recode(method, "'ps'='PS';'cbps'='CBPS';'svmLinear2'='SVM'"),
                CI = paste0(' (', round(lower, 2), ', ', round(upper, 2), ')'),
                timemen = factor(timemen, levels = c('0-10', '$<$10-20', '20+')))]
whi_cblb[, `:=`(upper = NULL, lower = NULL, B = NULL, gamma = NULL, subsets = NULL)]
setcolorder(whi_cblb, c('timemen', 'method', 'full', 'estim', 'se', 'CI', 'time'))
setorder(whi_cblb, timemen)

whi_cblb %>%
  kbl(digits = 2, escape = FALSE, booktabs = TRUE, 
      col.names = c('Time since menopause', 'Method', 'Full', 'Estim.', 'SE', 'CI', 'Med. Time (s)')) %>%
